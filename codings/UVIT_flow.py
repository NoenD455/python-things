        ...
        for i, block in enumerate(self.blocks):
            if i < half:
                ...
                if skip_idx < half:
                    ...
                    x = self.combine_projs[skip_idx](x)
                x = block(x)
...
class RectifiedFlowApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Rectified Flow U-ViT / U-GRU - Conditional Generation")
    ...
    def setup_model_tab(self):
        ...
        left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0,10))
        left_frame.pack_propagate(False)
        tk.Label(left_frame, text="Model Training (U-ViT / U-GRU)", font=("Arial",12,"bold")).pack(pady=(0,10))
    ...
    def apply_preset(self):
        ...
        if model_type == 'uvit':
            if preset in UVIT_PRESETS:
                cfg = UVIT_PRESETS[preset]
                ...
                self.log_model(f"Applied U-ViT preset '{preset}'")
            else:
                self.log_model(f"Unknown preset {preset}")
        else:
            if preset in UGRU_PRESETS:
                cfg = UGRU_PRESETS[preset]
                ...
                self.log_model(f"Applied U-GRU preset '{preset}'")
            else:
                self.log_model(f"Unknown preset {preset}")
    ...
    def initialize_model(self):
        ...
        try:
            ...
            if model_type == 'uvit':
                ...
                gru_num_layers = None
                if num_layers % 2 == 0:
                    self.log_model("Warning: U-ViT requires odd number of layers. Adjusting to next odd.")
                    num_layers += 1
                ...
                if gru_num_layers % 2 == 0:
                    self.log_model("Warning: U-GRU requires odd number of layers. Adjusting to next odd.")
                    gru_num_layers += 1
