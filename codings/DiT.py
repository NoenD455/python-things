...
class DiTApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Conditional DiT (Diffusion Transformer) with Full Hyperparams")
        ...
        # Settings variables (unified from flow)
        self.settings = {
        ...
        self.ode_method = tk.StringVar(value='ddpm')
    ...
    def setup_gui(self):
        ...
        self.notebook.add(self.dit_tab, text='Model Training')
        self.setup_dit_tab()
    ...
    def setup_dit_tab(self):
        ...
        tk.Button(prompt_frame, text="Generate Preview", command=self.preview_with_prompt, width=20).pack(pady=2)
    ...
    def apply_preset(self):
        ...
        if model_type == 'transformer':
            if preset in TRANSFORMER_PRESETS:
                ...
                self.log_dit(f"Applied Transformer preset '{preset}'")
            else:
                self.log_dit(f"Unknown preset {preset}")
        else:
            if preset in GRU_PRESETS:
                ...
                self.log_dit(f"Applied GRU preset '{preset}'")
            else:
                self.log_dit(f"Unknown preset {preset}")
    ...
    def setup_generation_tab(self):
        ...
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        tk.Label(main_frame, text="Generate Images (Diffusion)", font=("Arial",14,"bold")).pack(pady=(0,10))
        ...
        self.sampling_steps = tk.IntVar(value=50)
        ...
        method_combo = ttk.Combobox(ctrl_frame, textvariable=self.ode_method,
                                    values=['ddpm', 'ddim'], state='readonly', width=8)
    ...
    def initialize_model(self):
        ...
        try:
            ...
            if cond_enabled:
                ...
                self.dit_model.optimizer = self.model_optimizer
    ...
    def _update_progressive(self, grid_img, step_idx):
        ...
        self.inner_frame.update_idletasks()
        self.gen_canvas.configure(scrollregion=self.gen_canvas.bbox('all'))
        self.gen_info.config(text=f"Step {step_idx}/{self.sampling_steps.get()}")
