...
def adaptive_group_norm(channels):
    ...
    for g in range(32, 0, -1):
        if channels % g == 0:
            best_groups = g
            break
    return nn.GroupNorm(best_groups, channels)

def gaussian_blur_tensor(x, sigma):
    """Apply Gaussian blur to a batch of images (NCHW, values in [-1,1])."""
...
# ==================== UNet with adaptive GroupNorm ====================
class AttentionBlock(nn.Module):
    def __init__(self, dim, num_heads=4):
        super().__init__()
        self.norm = adaptive_group_norm(dim)
...
class DownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, time_emb_dim, cond_dim, dropout=0.1, has_attn=False):
        ...
        self.norm1 = adaptive_group_norm(out_channels)
        self.conv2 = nn.Conv2d(out_channels, out_channels, 3, padding=1)
        self.norm2 = adaptive_group_norm(out_channels)
...
class VelocityUNet(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, time_emb_dim=256, cond_dim=256,
        ...
        for i, mult in enumerate(channel_mult_used):
            ...
            block = DownBlock(cur_channels, out_channels, time_emb_dim, cond_dim, dropout, has_attn=use_attention)
        ...
        for i, mult in enumerate(rev_blocks):
            out_channels = base_channels * mult
            block = UpBlock(cur_channels + out_channels, out_channels, time_emb_dim, cond_dim, dropout, has_attn=use_attention)
        ...
        self.final_conv = nn.Sequential(
            adaptive_group_norm(cur_channels),
            nn.SiLU(),
...
# ==================== Rectified Flow Model (with training mode) ====================
class RectifiedFlowPixel:
    ...
    def _generate_source_from_target(self, x1):
        """Generate source x0 by applying task degradation to a clean image x1."""
    ...
    def _generate_source_from_noise(self, x1_shape):
        """Generate source x0 by applying task degradation to random noise."""
        ...
        if self.task == "noise":
            ...
            # Not meaningful for noise only - fallback to noise
            return noise
        elif self.task == "unquantization":
            bit_depth = self.task_params.get('bit_depth', 1)
            levels = 2 ** bit_depth
    ...
    def train_step(self, x1, cond=None, cfg_dropout_prob=0.0, update_ema=True, ema_decay=0.999):
    ...
    def sample(self, n_samples=16, cond=None, steps=50, method='euler', cfg_scale=1.0,
        ...
        def v_fn(t_val, x_val):
            ...
            if cfg_scale != 1.0:
                v_cond = self.model(x_val, t_tensor, cond=cond)
                v_uncond = self.model(x_val, t_tensor, cond=null_cond)
                return v_uncond + cfg_scale * (v_cond - v_uncond)
            else:
                return self.model(x_val, t_tensor, cond=cond)
...
class RectifiedFlowApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Optimal Transport Flow Matching")
    ...
    def setup_rectified_tab(self):
        ...
        tk.Label(left_frame, text="Optimal Transport Flow Training", font=("Arial",12,"bold")).pack(pady=(0,10))
    ...
    # ------------------------------------------------------------------
    # Flow to Image Tab (redesigned with image picker and grid)
    # ------------------------------------------------------------------
    def setup_flow_to_image_tab(self):
        ...
        # Top bar: prompt on left, image picker on right
    ...
    def run_flow_to_image(self):
        ...
        # Set progressive active flag if needed
    ...
    def _flow_images_thread(self, image_paths, cond, steps, method, cfg_scale, use_ema, degrade):
        try:
            ...
            n = len(imgs_tensors)
            # Build start_batch: either the original images or degraded versions
            if degrade:
    ...
    # ------------------------------------------------------------------
    # Core functions (dataset, training, generation)
    # ------------------------------------------------------------------
    ...
    def initialize_rectified_model(self):
        try:
            ...
            if self.settings['ema_enabled'].get():
                ...
                self.log_rectified(f"EMA enabled (decay={self.settings['ema_decay'].get()})")
            if cond_enabled:
                enc_type = self.settings['text_encoder_type'].get()
                if enc_type == 'BiGRU':
            ...
            for epoch in range(epochs):
                ...
                for images, text_tensors in loader:
                    ...
                    loss = self.rectified_model.train_step(x1, cond=cond, cfg_dropout_prob=cfg_dropout,
                                                           update_ema=use_ema, ema_decay=ema_decay)
                    epoch_loss += loss
                    batches += 1
    ...
    def save_rectified_model(self):
        ...
        if fname:
            save_dict = {
                ...
                'settings': {k:v.get() for k,v in self.settings.items() if k.startswith('rectified') or k in ['cfg_dropout_prob','text_encoder_type','text_encoder_size','ot_task','training_mode','blur_sigma','bit_depth','pixelation_size']},
    ...
    def load_rectified_model(self):
        ...
        try:
            ...
            if 'ema_shadow' in ckpt and self.rectified_model.ema:
                self.rectified_model.ema.shadow = ckpt['ema_shadow']
                self.log_rectified("EMA shadow restored.")
            self.log_rectified(f"Model loaded from {fname} (task: {task}, mode: {training_mode})")
        except Exception as e:
