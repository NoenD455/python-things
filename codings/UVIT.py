...
class UViTBlock(nn.Module):
    """Standard Pre-LN transformer block without time conditioning inside."""
...
class UViT(nn.Module):
    def __init__(self, latent_channels, patch_size, embed_dim, num_heads, num_layers,
                 mlp_ratio=4.0, dropout=0.1, time_emb_dim=256, cond_dim=None, use_conv=True):
        super().__init__()
        self.latent_channels = latent_channels
        ...
        self.patch_embed = nn.Conv2d(latent_channels, embed_dim, kernel_size=patch_size, stride=patch_size)
        ...
        self.blocks = nn.ModuleList([
            UViTBlock(embed_dim, num_heads, mlp_ratio, dropout) for _ in range(num_layers)
        ])
        ...
        self.final_linear = nn.Linear(embed_dim, patch_size * patch_size * latent_channels)
        if use_conv:
            self.final_conv = nn.Conv2d(latent_channels, latent_channels, 3, padding=1)
        else:
    ...
    def forward(self, z, t, cond=None):
        ...
        if cond is not None and self.cond_proj is not None:
            ...
            tokens = torch.cat([time_tokens, cond_tokens, x], dim=1)
            num_extra = 2
        else:
            tokens = torch.cat([time_tokens, x], dim=1)
            num_extra = 1
...
class UGRU(nn.Module):
    def __init__(self, latent_channels, patch_size, embed_dim, gru_hidden_dim, gru_num_layers,
                 dropout=0.1, time_emb_dim=256, cond_dim=None, use_conv=True):
        super().__init__()
        self.latent_channels = latent_channels
        self.patch_size = patch_size
        self.embed_dim = embed_dim
    ...
    def forward(self, z, t, cond=None):
        ...
        x = self.patch_embed(z).flatten(2).transpose(1, 2)
...
class LatentDiffusionUViT:
    def __init__(self, latent_channels, latent_h, latent_w, patch_size, embed_dim,
        ...
        if model_type.lower() == 'uvit':
            ...
            raise ValueError(f"Unknown model_type: {model_type} (use 'uvit' or 'ugru')")
...
class ConditionalImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if pil_img is None:
            pil_img = Image.new('RGB', (self.img_size, self.img_size), (0,0,0))
...
# U-ViT presets (same as before, but will be used to populate spinboxes)
UVIT_CONFIGS = {
    'tiny':   {'embed_dim': 64,  'num_heads': 2, 'num_layers': 5, 'mlp_ratio': 4.0},
    'small':  {'embed_dim': 128, 'num_heads': 4, 'num_layers': 7, 'mlp_ratio': 4.0},
    'medium': {'embed_dim': 192, 'num_heads': 6, 'num_layers': 9, 'mlp_ratio': 4.0},
    'large':  {'embed_dim': 256, 'num_heads': 8, 'num_layers': 11, 'mlp_ratio': 4.0},
}
...
class DiTApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Conditional U-ViT / U-GRU (Latent Diffusion) - Full Hyperparams")
        ...
        # Unified settings dictionary with all hyperparameters
        self.settings = {
            ...
            # U-ViT specific (visible when model_type='uvit')
            'uvit_embed_dim': tk.IntVar(value=128),
            'uvit_num_heads': tk.IntVar(value=4),
            'uvit_num_layers': tk.IntVar(value=7),
            'uvit_mlp_ratio': tk.DoubleVar(value=4.0),

            # U-GRU specific (visible when model_type='ugru')
            'ugru_embed_dim': tk.IntVar(value=256),
            'ugru_hidden_dim': tk.IntVar(value=512),
            'ugru_num_layers': tk.IntVar(value=2),
            ...
            'dit_use_conv': tk.BooleanVar(value=True),
    ...
    def setup_gui(self):
        ...
        self.notebook.add(self.dit_tab, text='U-ViT / U-GRU Training')
        self.setup_dit_tab()
    ...
    def setup_dit_tab(self):
        ...
        left_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0,10))
        left_frame.pack_propagate(False)
        tk.Label(left_frame, text="U-ViT / U-GRU Training", font=("Arial",12,"bold")).pack(pady=(0,10))
        ...
        tk.Button(left_frame, text="Initialize Model", command=self.initialize_dit_model, width=20).pack(pady=5)
        epoch_frame = tk.Frame(left_frame)
        epoch_frame.pack(pady=5)
        tk.Label(epoch_frame, text="Epochs:").pack(side=tk.LEFT)
        ...
        tk.Button(left_frame, text="Start Training", command=self.start_dit_training,
                  width=20, bg="lightgreen").pack(pady=5)
        tk.Button(left_frame, text="Stop Training", command=self.stop_dit_training,
                  width=20, bg="salmon").pack(pady=5)
        tk.Button(left_frame, text="Save Model", command=self.save_dit_model, width=20).pack(pady=5)
        tk.Button(left_frame, text="Load Model", command=self.load_dit_model, width=20).pack(pady=5)
        ...
        self.dit_preview_canvas = tk.Canvas(preview_canvas_frame, bg='gray', width=256, height=256)
        ...
        tk.Button(prompt_frame, text="Generate Preview", command=self.dit_preview_with_prompt, width=20).pack(pady=2)
    ...
    def setup_settings_tab(self):
        ...
        # Model type and patch size
        model_frame = tk.LabelFrame(scrollable_frame, text="U-ViT / U-GRU", padx=10, pady=10)
        ...
        # U-ViT parameters frame
        self.uvit_frame = tk.LabelFrame(model_frame, text="U-ViT Parameters", padx=5, pady=5)
        uvit_params = [
            ("Embed dim:", 'uvit_embed_dim', 32, 512, int),
            ("Num heads:", 'uvit_num_heads', 1, 16, int),
            ("Num layers:", 'uvit_num_layers', 2, 20, int),
            ("MLP ratio:", 'uvit_mlp_ratio', 1.0, 8.0, float),
        ]
        for label, key, low, high, typ in uvit_params:
        ...
        # U-GRU parameters frame
        self.ugru_frame = tk.LabelFrame(model_frame, text="U-GRU Parameters", padx=5, pady=5)
        ugru_params = [
            ("Embed dim:", 'ugru_embed_dim', 32, 512, int),
            ("GRU hidden dim:", 'ugru_hidden_dim', 32, 1024, int),
            ("GRU num layers:", 'ugru_num_layers', 1, 8, int),
        ]
        for label, key, low, high, typ in ugru_params:
        ...
        for label, key, low, high, typ in common_params:
        ...
        # Diffusion specific
        diff_frame = tk.LabelFrame(model_frame, text="Diffusion Hyperparameters", padx=5, pady=5)
        diff_frame.pack(fill=tk.X, pady=5)
        diff_params = [
            ("Timesteps (T):", 'dit_T', 50, 1000, int),
            ("Beta start:", 'dit_beta_start', 1e-5, 1e-2, float),
            ("Beta end:", 'dit_beta_end', 0.001, 0.1, float),
        ]
        for label, key, low, high, typ in diff_params:
            f = tk.Frame(diff_frame); f.pack(fill=tk.X, pady=2)
            tk.Label(f, text=label, width=20, anchor='w').pack(side=tk.LEFT)
            if typ == int:
                spin = ttk.Spinbox(f, from_=low, to=high, textvariable=self.settings[key], width=8)
            else:
            ...
            spin.pack(side=tk.RIGHT)
        f = tk.Frame(diff_frame); f.pack(fill=tk.X, pady=2)
        tk.Label(f, text="Beta schedule:", width=20, anchor='w').pack(side=tk.LEFT)
        sched_combo = ttk.Combobox(f, textvariable=self.settings['dit_beta_schedule'],
                                   values=['linear', 'cosine'], state='readonly', width=10)
        sched_combo.pack(side=tk.RIGHT)
        cb = tk.Checkbutton(diff_frame, text="Use final 3x3 conv", variable=self.settings['dit_use_conv'])
        cb.pack(anchor='w')
    ...
    def apply_preset(self):
        ...
        if model_type == 'uvit':
            if preset in UVIT_CONFIGS:
                cfg = UVIT_CONFIGS[preset]
                ...
                self.log_dit(f"Applied U-ViT preset '{preset}'")
            else:
                self.log_dit(f"Unknown U-ViT preset {preset}")
        else:
            if preset in UGRU_CONFIGS:
                cfg = UGRU_CONFIGS[preset]
                self.settings['ugru_embed_dim'].set(cfg['embed_dim'])
                self.settings['ugru_hidden_dim'].set(cfg['gru_hidden_dim'])
                self.settings['ugru_num_layers'].set(cfg['gru_num_layers'])
                self.log_dit(f"Applied U-GRU preset '{preset}'")
            else:
                self.log_dit(f"Unknown U-GRU preset {preset}")
    ...
    def apply_text_encoder_preset(self):
        ...
        self.log_dit(f"Applied {enc_type} {size} preset dims.")

    def setup_generation_tab(self):
        main_frame = tk.Frame(self.generation_tab)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        tk.Label(main_frame, text="Generate Images (U-ViT/U-GRU + VAE)", font=("Arial",14,"bold")).pack(pady=(0,10))
    ...
    def set_device(self):
        ...
        if self.dit_model:
            self.dit_model.device = self.device
            ...
            self.log_dit(f"Model moved to {dev_name}")
    ...
    def add_folder(self):
        ...
        self.log_dit(f"Added {count} images. Total: {len(self.image_paths)}")
    ...
    def load_csv(self):
        ...
        for full in self.image_paths:
            ...
            if base in path_by_basename:
                self.log_dit(f"Warning: duplicate basename '{base}', CSV match may be ambiguous")
            else:
        ...
        try:
            ...
            self.log_dit(f"Error reading CSV: {e}")
            return
        ...
        self.log_dit(f"CSV loaded. {len(self.image_paths)-unknown} images matched with labels.")
    ...
    def use_filenames_as_labels(self):
        ...
        self.log_dit("Using filenames as labels.")
    ...
    def use_folders_as_labels(self):
        ...
        self.log_dit("Using folder names as labels.")
    ...
    def initialize_dit_model(self):
        ...
        try:
            ...
            if model_type == 'uvit':
                ...
                gru_hidden_dim = self.settings['ugru_hidden_dim'].get()
                gru_num_layers = self.settings['ugru_num_layers'].get()
                num_heads = None
                num_layers = None
                mlp_ratio = None

            self.dit_model = LatentDiffusionUViT(
                latent_channels=latent_channels,
                latent_h=latent_h,
                latent_w=latent_w,
                patch_size=patch_size,
                ...
                use_conv=self.settings['dit_use_conv'].get(),
                gru_hidden_dim=gru_hidden_dim,
                gru_num_layers=gru_num_layers,
                cond_dim=cond_dim,
            ...
            if cond_enabled:
                ...
                self.dit_optimizer = optim.Adam(
                    list(self.dit_model.model.parameters()) + list(self.text_encoder.parameters()),
                    lr=self.settings['lr'].get()
                )
                self.dit_model.optimizer = self.dit_optimizer
                self.log_dit(f"Conditional {model_type.upper()} initialized, patch_size={patch_size}.")
            else:
                self.text_encoder = None
                self.log_dit(f"Unconditional {model_type.upper()} initialized, patch_size={patch_size}.")
        except Exception as e:
            self.log_dit(f"Init error: {e}")
    ...
    def start_dit_training(self):
        ...
        if not self.dit_model:
            self.log_dit("Model not initialized!")
            return
        if not self.vae_model:
            self.log_dit("No VAE!")
            return
        if self.training_dit:
            self.log_dit("Already training.")
            return
        cond_enabled = self.settings['cond_enabled'].get()
        if cond_enabled and not self.labels:
            self.log_dit("Conditional training requires labels. Load CSV or use filenames/folders.")
            return
        try:
        ...
        thread.start()
        self.log_dit(f"Training started for {epochs} epochs.")
            ...
            for epoch in range(epochs):
                ...
                for images, text_tensors in loader:
                    ...
                    if cond_enabled and self.text_encoder is not None:
                        ...
                        cond = None
                    loss = self.dit_model.train_step(latents, cond=cond, cfg_dropout_prob=cfg_dropout)
                    epoch_loss += loss
                    batches += 1
                avg_loss = epoch_loss / batches if batches else 0
                ...
                if (epoch+1) % 5 == 0:
                    self.dit_preview_with_prompt()
            self.training_dit = False
            self.log_dit("Training finished.")
        except Exception as e:
            self.log_dit(f"Training error: {e}")
            import traceback
            traceback.print_exc()
            self.training_dit = False

    def dit_preview_with_prompt(self):
        if not self.dit_model or not self.vae_model:
            self.log_dit("Models not loaded for preview")
            return
        try:
            ...
            if not unconditional and self.text_encoder is not None:
                ...
                self.log_dit(f"Preview: conditional with prompt '{prompt}'")
            elif not unconditional:
                self.log_dit("Preview: conditioning disabled, generating unconditional")
                unconditional = True
            else:
                self.log_dit("Preview: unconditional generation")

            z = self.dit_model.sample(n_samples=16, steps=50, method='ddim', cond=cond, cfg_scale=cfg_scale)
    ...
    def stop_dit_training(self):
        self.training_dit = False
        self.log_dit("Training stopped.")

    def save_dit_model(self):
        if not self.dit_model:
            self.log_dit("No model.")
            return
        fname = filedialog.asksaveasfilename(defaultextension=".pth", filetypes=[("PyTorch","*.pth")])
        if fname:
            save_dict = {
            ...
            self.log_dit(f"Model saved to {fname}")
    ...
    def load_dit_model(self):
        ...
        try:
            ...
            if 'settings' in ckpt:
                for k, v in ckpt['settings'].items():
                    if k in self.settings:
                        self.settings[k].set(v)
            if not self.vae_model:
                self.log_dit("Please load VAE first.")
                return
            ...
            if 'T' in ckpt:
                ...
                self.dit_model.betas = ckpt['betas'].to(self.device)
                self.dit_model.alphas = ckpt['alphas'].to(self.device)
                self.dit_model.alpha_bars = ckpt['alpha_bars'].to(self.device)
            if 'text_encoder_state' in ckpt and self.text_encoder is not None:
                self.text_encoder.load_state_dict(ckpt['text_encoder_state'])
            self.log_dit(f"Model loaded from {fname}")
        except Exception as e:
    ...
    def _generate_thread(self, n, steps, method, cond, cfg_scale):
        try:
            z = self.dit_model.sample(n_samples=n, steps=steps, method=method, cond=cond, cfg_scale=cfg_scale)
            target_size = (self.settings['img_size'].get(), self.settings['img_size'].get())
            with torch.no_grad():
                samples = self.vae_model.decode(z, target_size=target_size)
            samples = (samples + 1) / 2
    ...
    def _progressive_thread(self, n, steps, method, cond, cfg_scale):
        try:
            ...
            generator = self.dit_model.sample_step_by_step(n_samples=n, steps=steps, method=method, cond=cond, cfg_scale=cfg_scale)
            step_count = 0
            target_size = (self.settings['img_size'].get(), self.settings['img_size'].get())
