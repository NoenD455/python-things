...
# ==================== Sinusoidal Position Embeddings (1D and 2D) ====================
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    omega = 1.0 / (10000 ** (2 * omega / embed_dim))          # (embed_dim//2,)
    out = pos[:, None] * omega[None, :]                       # (N, embed_dim//2)
...
def get_2d_sincos_pos_embed(embed_dim, grid_h, grid_w):
    ...
    """
    # We split embed_dim into two halves for height and width
    half_dim = embed_dim // 2
    dim_h = half_dim
    dim_w = embed_dim - half_dim  # in case embed_dim is odd, width gets the extra

    # positions: 0..grid_h-1, 0..grid_w-1
    ...
    # Expand to grid_h x grid_w and concatenate
    emb_h = emb_h.unsqueeze(1)  # (grid_h, 1, dim_h)
    emb_w = emb_w.unsqueeze(0)  # (1, grid_w, dim_w)

    # Broadcast to full grid
    emb_h = emb_h.expand(grid_h, grid_w, -1)  # (grid_h, grid_w, dim_h)
    emb_w = emb_w.expand(grid_h, grid_w, -1)  # (grid_h, grid_w, dim_w)

    pos_embed = torch.cat([emb_h, emb_w], dim=-1)  # (grid_h, grid_w, embed_dim)
    pos_embed = pos_embed.flatten(0, 1).unsqueeze(0)  # (1, grid_h*grid_w, embed_dim)
    return pos_embed
...
class FinalLayer(nn.Module):
    def __init__(self, hidden_size, output_dim):
        """
        output_dim: the dimension to project each token to (e.g., embed_dim for later conv unembedding)
        """
        super().__init__()
...
class DiT(nn.Module):
    def __init__(self, latent_channels, patch_size, embed_dim, num_heads, num_layers,
        ...
        # Remove learned pos_embed; we'll compute 2D sinusoidal on the fly
        ...
        # Final layer: project to embed_dim (so we can reshape and apply transposed conv)
        self.final_layer = FinalLayer(embed_dim, embed_dim)
        # Exit convolution: up-sample from embed_dim channels to latent_channels
        self.unembed = nn.ConvTranspose2d(embed_dim, latent_channels,
                                          kernel_size=patch_size, stride=patch_size)

    def forward(self, z, t, cond=None):
        B, C, H, W = z.shape
        x, Hp, Wp = self.patch_embed(z)          # (B, N, embed_dim)
        N = Hp * Wp

        # 2D sinusoidal positional embedding
        pos_embed = get_2d_sincos_pos_embed(self.embed_dim, Hp, Wp).to(x.device)  # (1, N, embed_dim)
        x = x + pos_embed

        c = self.time_embedder(t, cond=cond)    # (B, embed_dim)

        for block in self.blocks:
            x = block(x, c)                     # (B, N, embed_dim)

        x = self.final_layer(x, c)              # (B, N, embed_dim)

        # Reshape to feature map: (B, embed_dim, Hp, Wp)
        x = x.transpose(1, 2).view(B, self.embed_dim, Hp, Wp)
        # Apply transposed conv to get (B, latent_channels, H, W)
        out = self.unembed(x)
        return out
...
class GRUModel(nn.Module):
    def __init__(self, latent_channels, patch_size, embed_dim, time_emb_dim,
        ...
        # Final layer: project to embed_dim
        self.final_layer = FinalLayer(embed_dim, embed_dim)
        # Exit convolution
        self.unembed = nn.ConvTranspose2d(embed_dim, latent_channels,
                                          kernel_size=patch_size, stride=patch_size)
    ...
    def forward(self, z, t, cond=None):
        ...
        x, _ = self.gru(x)                       # (B, N, gru_hidden_dim)
        x = self.out_proj(x)                     # (B, N, embed_dim)
