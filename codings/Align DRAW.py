...
class AlignDRAW(nn.Module):
    def __init__(self, in_channels=3, img_size=32, text_encoder=None, latent_dim=64, hidden_dim=256, num_steps=16, patch_size=12, decoder_type='LSTM', device='cpu'):
        ...
        # Decoder input size = latent_dim + word_feat_dim (context dimension)
        decoder_input_size = latent_dim + self.word_feat_dim

        if decoder_type == 'LSTM':
            self.decoder_cell = nn.LSTMCell(input_size=decoder_input_size, hidden_size=hidden_dim)
        else:
            self.decoder_transformer = TransformerImageDecoder(hidden_dim, latent_dim, self.word_feat_dim)
    ...
    def forward(self, images, text_tensors, teacher_forcing=True):
        ...
        for step in range(self.num_steps):
            if self.use_conditioning and text_tensors is not None:
                ...
                context = torch.zeros(B, self.word_feat_dim, device=device)
