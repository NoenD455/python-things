...
# fixed EMA (target model), two EMA modes (Standard / Lookahead), and live parameter tuning.
...
# ==================== Transformer Mid‑Block and Skip Modulation ====================
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x, cond_emb):
        # Self-attention
        norm_x = self.norm1(x)
        attn_out = self.self_attn(norm_x, norm_x, norm_x)[0]
        x = x + self.dropout(attn_out)

        # Cross-attention with cond (if provided)
        if cond_emb is not None:
            norm_x = self.norm2(x)
            attn_out = self.cross_attn(norm_x, cond_emb, cond_emb)[0]
            x = x + self.dropout(attn_out)
...
class TransformerMidBlock(nn.Module):
    def __init__(self, channels, time_emb_dim, cond_dim, num_layers=2, num_heads=4, dropout=0.1):
        super().__init__()
        ...
        # Cond projection for cross-attention
        self.cond_proj = nn.Linear(cond_dim, embed_dim) if cond_dim > 1 else None
    ...
    def forward(self, x, t_emb, cond):
        B, C, H, W = x.shape
        # Embed via 3x3 conv
        x = self.conv_in(x)  # (B, embed_dim, H, W)

        # Add time embedding
        time_out = self.time_mlp(t_emb)  # (B, embed_dim)
        time_out = time_out[:, :, None, None]  # (B, embed_dim, 1, 1)
        x = x + time_out
        ...
        x = self.conv_out(x)  # back to original channels
        return x

class SkipModulationMLP(nn.Module):
    """
    5‑layer MLP (1x1 convs) that modulates skip connections.
    The middle layer has dimension = cond_dim (or 64 if unconditional)
    and receives the prompt embedding as an additional input.
    """
    def __init__(self, channels, cond_dim):
        super().__init__()
        self.channels = channels
        self.cond_dim = cond_dim
        # Middle dimension: use cond_dim if >1 else 64
        mid_dim = cond_dim if cond_dim > 1 else 64
        self.mid_dim = mid_dim

        hidden = max(channels, 64)  # hidden size for first layers

        self.conv1 = nn.Conv2d(channels, hidden, kernel_size=1)
        self.conv2 = nn.Conv2d(hidden, hidden, kernel_size=1)

        # Middle layer: if conditional, concatenate cond along channel
        if cond_dim > 1:
            self.conv3 = nn.Conv2d(hidden + cond_dim, mid_dim, kernel_size=1)
        else:
            self.conv3 = nn.Conv2d(hidden, mid_dim, kernel_size=1)

        self.conv4 = nn.Conv2d(mid_dim, hidden, kernel_size=1)
        self.conv5 = nn.Conv2d(hidden, channels, kernel_size=1)
    ...
    def forward(self, x, cond):
        ...
        # Middle layer with cond
        if self.cond_dim > 1 and cond is not None:
            cond_exp = cond[:, :, None, None].expand(-1, -1, H, W)  # (B, D, H, W)
            out_cat = torch.cat([out, cond_exp], dim=1)             # (B, hidden+D, H, W)
            out = self.act(self.conv3(out_cat))
        else:
            out = self.act(self.conv3(out))

        out = self.act(self.conv4(out))
        out = self.conv5(out)  # back to original channels
        return out
...
class VelocityUNet(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, time_emb_dim=256, cond_dim=256,
        ...
        # --- Replace mid blocks with Transformer + cross-attention ---
        self.transformer = TransformerMidBlock(cur_channels, time_emb_dim, cond_dim,
                                               num_layers=2, num_heads=4)

        # --- Skip modulation MLPs for each down block ---
        self.skip_mods = nn.ModuleList()
        for mult in channel_mult_used:
            out_channels = base_channels * mult
            self.skip_mods.append(SkipModulationMLP(out_channels, cond_dim))
    ...
    def forward(self, x, t, cond=None):
        ...
        # Middle Transformer (cross‑attention to cond)
        x = self.transformer(x, t_emb, cond)

        # Up path, applying skip modulation before concatenation
        # Reverse skip_mods to match the order of popping (deepest first)
        skip_mods_rev = list(reversed(self.skip_mods))
        mod_idx = 0
        for layer in self.ups:
            if isinstance(layer, UpBlock):
                skip = skips.pop()
                # Apply modulation to the skip connection
                skip = skip_mods_rev[mod_idx](skip, cond)
                mod_idx += 1
                # Crop skip to match spatial dimensions of x if needed
...
class RectifiedFlowApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Rectified Flow - Unified (fixed EMA, two modes, live tuning)")
    ...
    # ------------------------------------------------------------------
    # Settings Tab (with EMA mode dropdown and live tuning support)
    # ------------------------------------------------------------------
    ...
    def initialize_rectified_model(self):
        try:
            ...
            if cond_enabled:
                ...
                self.log_rectified(f"Conditional model with {enc_type} encoder, task={task}, mode={training_mode}, padding={padding_mode}")
            else:
                self.text_encoder = None
                self.log_rectified(f"Unconditional model, task={task}, mode={training_mode}, padding={padding_mode}")
        except Exception as e:
    ...
    def train_rectified_loop(self, epochs):
        try:
            ...
            for epoch in range(epochs):
                ...
                # Update EMA settings if changed
    ...
    def save_rectified_model(self):
        ...
        if fname:
            ...
            save_dict = {
                ...
                'settings': {k:v.get() for k,v in self.settings.items() if k.startswith('rectified') or k in ['cfg_dropout_prob','text_encoder_type','text_encoder_size','ot_task','training_mode','blur_sigma','bit_depth','pixelation_size','padding_mode','ema_enabled','ema_decay','ema_mode']},
    ...
    def load_rectified_model(self):
        ...
        try:
            ...
            # If EMA is enabled after loading, we need to sync ema_model with loaded model
            if self.rectified_model.use_ema and self.rectified_model.ema_model is not None:
                # Since we loaded the model_state (which is the target), we need to set ema_model to that as well.
                # However, we also need online model. The loaded model_state might be EMA or online?
                # We loaded into model (online). For consistency, if EMA enabled, we should copy the loaded weights to ema_model
                # and also keep online as is (or we could set online = ema if we want to start from target).
                # To be safe, we set ema_model to the same as model (which is now the loaded target).
            ...
            self.log_rectified(f"Model loaded from {fname} (task: {self.settings['ot_task'].get()}, mode: {self.settings['training_mode'].get()}, padding: {self.settings['padding_mode'].get()})")
        except Exception as e:
